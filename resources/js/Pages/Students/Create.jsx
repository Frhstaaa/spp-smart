...
export default function Create({ auth, classes, angkatans }) {
    ...
    return (
        <MainLayout user={auth.user} header={<h2 className="font-bold text-2xl text-slate-800 dark:text-slate-200">Tambah Siswa Baru</h2>}>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-3xl border border-slate-100 dark:border-slate-700/50">
                            <div className="p-4 md:p-8">
                                ...
                                <div className="bg-teal-50 dark:bg-teal-900/20 p-5 rounded-2xl border border-teal-100 dark:border-teal-900/50 mb-6">
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 p-5 rounded-2xl border border-slate-200 dark:border-slate-700/50">
                                    <h4 className="font-bold text-slate-800 dark:text-slate-300 mb-2">Pembuatan Akun Portal (Opsional)</h4>
                                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Sistem akan otomatis membuat akun untuk login siswa jika email diisi. Password default adalah NIS.</p>
                                    
                                    <div>
                                        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Email Aktif</label>
                                        <input type="email" value={data.email} onChange={e => setData('email', e.target.value)} className="w-full rounded-xl border-slate-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 dark:bg-slate-900 dark:border-slate-700 dark:text-white" placeholder="contoh: siswa@gmail.com" />
