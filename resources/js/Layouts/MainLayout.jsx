...
const NavItem = ({ href, icon: Icon, label, active, onClick, themeC }) => (
 ...
 >
 <span className={`flex items-center justify-center w-8 h-8 rounded-lg transition-all duration-300 shrink-0 ${
 active ? `${themeC.iconBg} text-white shadow-md ${themeC.iconShadow}` : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300'
 }`}>
...
const BottomNavItem = ({ href, icon: Icon, label, active, onClick, themeC }) => {
    ...
    return (
        ...
        >
            <div className={`flex flex-col items-center justify-center gap-1 w-full transition-colors ${
                active ? '' : 'hover:bg-slate-100/30 dark:hover:bg-slate-800/30 rounded-xl'
            }`}>
                <div className={`p-1.5 rounded-full transition-all duration-300 ${active ? themeC.bottomNavActiveBg : ''}`}>
                    <Icon className={`w-6 h-6 transition-colors duration-300 ${
                        active ? `${themeC.bottomNavActiveIcon} stroke-[2.5]` : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300 stroke-[2]'
                    }`} />
                </div>
                
                <span className={`text-[9px] sm:text-[10px] font-bold ${
                    active ? themeC.bottomNavActiveText : 'text-slate-400 dark:text-slate-500'
                } whitespace-nowrap`}>
                    {label}
                </span>
            </div>
...
export default function MainLayout({ children }) {
 ...
 Developed by <span className={themeC.bottomNavActiveText}>Tiyas Frahesta</span>
 </div>
 </main>
 </div>
