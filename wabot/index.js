...
app.post('/api/send', async (req, res) => {
    try {
        ...
        if (!target || !message) {
            return res.status(400).json({ status: false, message: 'Target dan message diperlukan' });
        }
        
        // Format nomor target
        let formattedTarget = target.toString();
        // Replace all non-digit characters except +
        ...
        if (!formattedTarget.endsWith('@c.us')) {
            formattedTarget += '@c.us';
        }
        
        await client.sendMessage(formattedTarget, message);
        console.log(`✅ Pesan API terkirim ke ${formattedTarget}`);
